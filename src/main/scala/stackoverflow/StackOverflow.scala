...
object StackOverflow extends StackOverflow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //val getMore = getQuestionsMoreScore(scored)
    //println(getMore.foreach(println))
...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    def answerHighScore(as: Array[Answer]): HighScore = {
     ...
     grouped.flatMap(_._2).groupByKey().mapValues(values => answerHighScore(values.toArray))
  }

  def getQuestionsMoreScore(questions: RDD[(Question, HighScore)]) = {

    questions.sortBy(_._2, ascending = false).take(5)
}
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean): Array[(Int, Int)] = {
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langLabel: String   = langs(langMode / langSpread) // most common language in the cluster
      val clusterSize: Int = vs.size
      val langPercent: Double = vs.filter(_._1 == langMode).size / clusterSize * 100 // percent of the questions in the most common language
